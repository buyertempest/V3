...
TIMES="10"
CHATID="6466237592"
KEY="6635824936:AAHwQRbCySP8AlQFcA_LNTB1RR_2wy56TT4"
...
clear;clear;clear
echo -e " ${CYAN} ┌─────────────────────────────────────────────────┐${NC}"
echo -e " ${CYAN} │ ${BG_RED}              TEMPEST TUNNELING V3                  ${NC} ${CYAN}│$NC"
echo -e " ${CYAN} └─────────────────────────────────────────────────┘${NC}"
echo ""
...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/buyertempest/V3/main/REGIST | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/buyertempest/V3/main/REGIST | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/buyertempest/V3/main/REGIST | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e "\e[32mloading...\e[0m"
clear
REPO="https://raw.githubusercontent.com/buyertempest/V3/main/"
start=$(date +%s)
secs_to_human() {
...
clear
restart_system() {
USRSC=$(wget -qO- https://raw.githubusercontent.com/buyertempest/V3main/REGIST | grep $ipsaya | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/buyertempest/V3/main/REGIST | grep $ipsaya | awk '{print $3}')
...
wget raw.githubusercontent.com/buyertempest/V3/main/file/limit.sh && chmod +x limit.sh && ./limit.sh
cd
...
ExecStart=/usr/bin/file-ip vmip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
[Service]
WorkingDirectory=/root
ExecStart=/usr/bin/file-ip vlip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
[Service]
WorkingDirectory=/root
ExecStart=/usr/bin/file-ip trip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
print_success "file Quota Service"
}
function ssh_slow(){
clear
